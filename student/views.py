...
def student_dashboard(request):
    try:
        ...
        lessons_from_class = Lesson.objects.filter(school_class=school_class) if school_class else Lesson.objects.none()

        lessons_from_groups = Lesson.objects.filter(study_group__in=student.study_groups.all())


        lessons = (lessons_from_class | lessons_from_groups).distinct().order_by('-date')
